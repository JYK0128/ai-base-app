...
import { TermsGroupType } from '@pkg/database';
...
export class TermsController {
  ...
  async createDocument(@Payload() data: { groupType: TermsGroupType, code: string, title: string, required?: boolean, organizationId?: string }) {
  ...
  async createVersion(@Payload() data: { termsDocumentId: string, versionLabel: string, contentMd: string, publish?: boolean }) {
  ...
  async agreeTerms(@Payload() data: { userId: string, termsVersionId: string, organizationId?: string, source?: string, ipAddress?: string, userAgent?: string }) {
