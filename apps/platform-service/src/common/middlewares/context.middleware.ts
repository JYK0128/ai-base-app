...
export class ContextMiddleware implements NestMiddleware {
  ...
  private setUserContext(req: AppRequest) {
    ...
    try {
      ...
      const accountId = getStringClaim(payload.accountId);
      if (accountId) {
        this.cls.set('accountId', accountId);
      }
      const memberId = getStringClaim(payload.memberId);
      if (memberId) {
        this.cls.set('memberId', memberId);
      }
      const organizationId = getStringClaim(payload.organizationId);
      if (organizationId) {
