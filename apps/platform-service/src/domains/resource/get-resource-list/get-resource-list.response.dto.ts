...
export class GetResourceListItem extends EntityResponseType(Resource) {
  ...
  @ApiProperty({ type: () => [GetResourceListItem], description: '하위 리소스 목록' })
  override children!: GetResourceListItem[];
}
export class GetResourceListResponseDto extends ListResponseDto<GetResourceListItem> {
  constructor(args: GetResourceListResponseDto) {
    super();
    this.items = args.items;
  }

  @ApiProperty({ type: () => [GetResourceListItem], description: '리소스 목록' })
  items!: GetResourceListItem[];
}
