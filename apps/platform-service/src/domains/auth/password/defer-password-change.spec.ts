...
import { DeferPasswordChangeUseCase } from './defer-password-change.use-case';

describe('DeferPasswordChangeUseCase', () => {
  ...
  it('extends the next password change date by 90 days', async () => {
    ...
    const handler = new DeferPasswordChangeUseCase(repository as never);

    await handler.execute({ accountId: 'user-1' });
  ...
  it('throws when the account cannot be found', async () => {
    ...
    await expect(
      handler.execute({ accountId: 'missing-user' }),
