...
본 문서는 Feature-First(기능 중심) 구조에 기반한 파일 배치와 역할 분리 기준을 정의합니다. 과도한 폴더 깊이를 지양하고, 파일명과 경로만 보고도 역할을 한눈에 파악할 수 있도록 유지합니다.

---

## 📂 1. Feature-First 레이아웃

모든 CQRS 관련 파일은 기술적 역할(Controllers, Handlers 등)로 나누지 않고, **도메인 피처(Feature)** 단위로 그룹화하여 관리합니다.

* 단일 유스케이스와 관련된 모든 계약, DTO, 핸들러, 에러 정의 파일은 동일한 디렉토리에 위치합니다.
* 이를 통해 특정 기능을 수정할 때 컨텍스트 스위칭을 최소화합니다.
...
일관된 명명 규칙을 적용하여 파일의 기술적 역할과 비즈니스 책임을 명확히 구분합니다.
...
핸들러 및 퍼블리셔 등의 의존성은 도메인 모듈(예: `MembersModule`)의 `providers` 배열에 명시적으로 등록해야 NestJS CQRS가 정상적으로 감지할 수 있습니다.
