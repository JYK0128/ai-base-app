...
1. **디렉토리 생성**: 프로젝트 성격에 따라 `apps/`, `packages/`, `web/`, `mobile/` 하위에 폴더 생성
2. **템플릿 복사**: `base-package` 내의 모든 파일을 신규 디렉토리로 복사
3. **package.json 최적화**: 프로젝트명 수정 및 버전 초기화 (`0.1.0`)
4. **루트 설정 업데이트**: 빌드 가능한 TypeScript 프로젝트 경로를 루트 `tsconfig.json`의 `references`에 추가
5. **워크스페이스 등록**: 루트 디렉토리에서 `pnpm install` 실행하여 의존성 동기화
6. **헬스체크 구성**: 프로젝트 성격에 맞는 `/health/live`, `/health/ready` 및 probe 전략을 구성
7. **무결성 검증**: `pnpm --filter=<package-name> lint`와 `pnpm --filter=<package-name> build`를 실행하여 설정 정상 작동 확인
