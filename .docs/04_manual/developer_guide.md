...
* 파일 네이밍은 도메인 접두사 기반으로 통일함. (예: `member.account.entity.ts`, `organization.permission.entity.ts`, `organization.role.entity.ts`)
* 관계 엔티티도 명시적으로 분리하여 관리함. (예: `OrganizationRoleAssignment`, `TermsConsent`)
...
* 시더는 `DatabaseSeeder`를 루트 진입점으로 사용하고, 각 세부 시더를 조합해 초기 데이터를 구성함.
