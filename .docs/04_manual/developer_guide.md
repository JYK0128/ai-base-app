...
* 권한 체크/캐싱/도메인 로직은 `packages/utils`의 `DatabaseRbacManager`에서 처리함.
* DB 영속화 구현(`MikroOrmRbacRepository`, `MikroOrmI18nMessageRepository`)은 `packages/database/src/repositories`에서 처리함.
